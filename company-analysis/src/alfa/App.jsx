...
export default function AlphaApp() {
  ...
  const [pickerOpen, setPickerOpen] = useState(false), [activeMatch, setActiveMatch] = useState(-1);
  ...
  const visibleMatches = queryReady ? matches.slice(0, 8) : [];
  const showMatches = pickerOpen && visibleMatches.length > 0;
  useEffect(() => { setActiveMatch(-1); }, [query]);
  function select(item) {
    setSymbol(item.symbol); setTab('Resumen'); setQuery(companyDisplayName(item)); setPickerOpen(false); setActiveMatch(-1);
    const url = new URL(location.href); url.searchParams.set('symbol', item.symbol); history.replaceState(null, '', url);
  }
  function navigateMatches(event) {
    if (!queryReady || !visibleMatches.length) return;
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      event.preventDefault(); setPickerOpen(true);
      setActiveMatch(index => event.key === 'ArrowDown'
        ? (index + 1) % visibleMatches.length
        : (index <= 0 ? visibleMatches.length - 1 : index - 1));
    } else if (event.key === 'Enter' && showMatches && activeMatch >= 0) {
      event.preventDefault(); select(visibleMatches[activeMatch]);
    } else if (event.key === 'Escape' && pickerOpen) {
      event.preventDefault(); setPickerOpen(false); setActiveMatch(-1);
    }
  }
  return <div className="alpha-root">
    ...
    <main className="main">
      <section className="alpha-picker screen-only" aria-labelledby="alpha-search-title">
        ...
        <label htmlFor="alpha-query">Nombre, ticker, ISIN o identificador</label>
        <div className="alpha-combobox">
          <input type="search" id="alpha-query" className="search-input" role="combobox" aria-autocomplete="list"
            aria-expanded={showMatches} aria-controls="alpha-company-options"
            aria-activedescendant={showMatches && activeMatch >= 0 ? `alpha-company-option-${activeMatch}` : undefined}
            autoComplete="off" value={query}
            onFocus={() => { if (queryReady) setPickerOpen(true); }}
            onChange={e => { setQuery(e.target.value); setPickerOpen(Boolean(e.target.value.trim())); }}
            onKeyDown={navigateMatches} placeholder="Por ejemplo, Iberdrola, IBE.MC o su ISIN" />
          {showMatches && <div id="alpha-company-options" className="alpha-combobox__options" role="listbox" aria-label="Empresas coincidentes">
            {visibleMatches.map((item, index) => <button id={`alpha-company-option-${index}`} key={item.assetId} type="button" role="option"
              aria-selected={index === activeMatch} tabIndex={-1} className={index === activeMatch ? 'is-active' : ''}
              onMouseEnter={() => setActiveMatch(index)} onMouseDown={event => event.preventDefault()} onClick={() => select(item)}>
              <strong>{companyDisplayName(item)}</strong><span>{item.symbol} · {item.isin}</span>
            </button>)}
          </div>}
        </div>
        ...
        {snapshot && <>
          {!queryReady && <p role="status">{snapshot.entries.length} empresas disponibles. Escribe para buscar y elegir una.</p>}
          {queryReady && matches.length > 0 && <p role="status">{matches.length} {matches.length === 1 ? 'coincidencia' : 'coincidencias'} de {snapshot.entries.length}{matches.length > visibleMatches.length ? ` · Mostrando las primeras ${visibleMatches.length}; escribe más para acotar.` : ' · Elige una en el desplegable.'}</p>}
          {queryReady && !matches.length && <p role="status">No hay coincidencias. Prueba con otro nombre, ticker, ISIN o identificador.</p>}
        </>}
      </section>

...
test('búsqueda por nombre, símbolo e ISIN sin criterios de atractivo', () => {
  ...
  for (const query of ['Émpresa', 'TEST.MC', asset.isin, asset.asset_id, 'empresa test']) assert.equal(searchCompanies(entries, query).length, 1);
...
test('integración: carga empresas al seleccionarlas y conserva el iframe al cambiar de vista', async () => {
  ...
  assert.equal(suite.nodes.get('analysis-title').textContent, 'Mi cartera');
  ...
  assert.equal(suite.nodes.get('analysis-title').textContent, 'Análisis y valoración de empresas');
  assert.equal(suite.nodes.get('analysis-current').textContent, 'Análisis y valoración de empresas');
  assert.match(suite.nodes.get('analysis-lead').textContent, /fundamentales, el análisis técnico/);
  ...
  assert.equal(suite.nodes.get('analysis-title').textContent, 'Carteras modelo');

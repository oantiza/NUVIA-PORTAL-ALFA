(() => {
  ...
  const areaForRoute = (currentRoute) => {
    ...
    if (currentRoute === 'colaboradores.html') return 'colaboradores';
    return location.hash === '#que-es-nuvia' || currentRoute === 'que-es-nuvia.html' ? 'nuvia' : 'inicio';
  };

  const pageKindForRoute = (currentRoute) => {
    if (currentRoute === 'index.html') return 'home';
    if (['economia.html', 'patrimonio.html', 'bienestar.html'].includes(currentRoute)) return 'home';
